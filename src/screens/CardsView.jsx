    ...
    <div style={{ minHeight: "100vh", background: "#F9FAFB", padding: "2rem 1.5rem", fontFamily: "system-ui, -apple-system, sans-serif" }}>
      <div style={{ maxWidth: 580, margin: "0 auto" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: "1rem" }}>
          ...
          <span style={{ background: cp.lit, color: cp.txt, border: `1px solid ${cp.med}`, borderRadius: 20, padding: "3px 12px", fontSize: 12, fontWeight: 600 }}>{topic}</span>
        ...
        <div style={{ display: "flex", gap: 10, marginTop: "1.25rem" }}>
          <button onClick={() => dispatch({ type: "SET", payload: { topic: topics[topicIdx - 1] } })} disabled={!hasPrev}
          ...
          <button onClick={() => dispatch({ type: "SET", payload: { topic: topics[topicIdx + 1] } })} disabled={!hasNext}
